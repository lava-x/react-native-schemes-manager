...
function updateProject(project) {
	...
	for (const key of Object.keys(section)) {
		...
		if (
			step &&
			step.shellScript &&
			step.shellScript.indexOf("react-native-xcode.sh") >= 0
		) {
			// Found it!
			...
			const scriptSettings =
				(mappings.settings && mappings.settings["fix-script"]) || {};
			const nodeCommand = scriptSettings.nodeCommand
				? scriptSettings.nodeCommand + " "
				: "";
			const env = scriptSettings.env || [];

			const devConfigs = `\\"+(${configurations}${
				configurations.length ? "|" : ""
			}Debug)\\"`;
			const extraArgs = `\\"--white-label \${WHITE_LABEL\}\\"`;
			env.CLI_PATH = "node_modules/haul/bin/cli.js";
			...
			const commands = [...exports, runCommand];
			...
			if (step.shellScript === newScript) {
				...
				console.log(
					chalk.gray(
						` - [fix-script]: ${path.dirname(
							path.relative(process.cwd(), project.filepath)
						)} skipped`
					)
				);
				return false;
				...
				console.log(
					chalk.gray(
						` ${chalk.green("✔")} [fix-script]: ${path.dirname(
							path.relative(process.cwd(), project.filepath)
						)} ${chalk.green("fixed")}`
					)
				);
